...
export default function Register() {
  ...
  useEffect(() => {
    fetch('http://localhost:8088/api/ads/get')
    .then(res => res.json())
      .then(value => {
...
let random = Math.floor(Math.random() * 5);
  ...
  const formik = useFormik({
    ...
    onSubmit: values => {
            ...
            fetch('http://localhost:8087/api/natwestdb/add',
              {
                method: 'POST',
        ...
        <div className='section2'>
          ...
          <div className='form'>
            ...
            <p className='mobile_register'>Enter your Email ID:</p>
              ...
              <p className='mobile_register'>Re-enter your password:</p>
              <div>
                <input type='password' className='input' name='Re-enter your password:' id='password' onSubmit={formik.handleSubmit} onChange={formik.handleChange} ></input>
                <label className='form_placeholder'>Re-enter your password:<span className='mandatory'>*</span></label>
                {
                  formik.errors.password && formik.touched.password ? <div className='red'>{formik.errors.password}</div> : null
                }
              </div>
